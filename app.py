...
def _wipe_dir(path: str):
    """Delete every file directly inside a directory. Used at startup:
    the in-memory JOBS registry is reset on every process restart, so any
    file already sitting in uploads/ or outputs/ at that point is guaranteed
    orphaned (nothing can reference it anymore) and safe to delete. Without
    this, files left behind by an interrupted upload/process (e.g. a crash
    or an OOM-triggered restart that reuses the same disk) accumulate across
    restarts and can themselves push memory-constrained hosts over the edge
...
# An upload that's started but abandoned (browser tab closed, network died
# mid-upload) leaves its partial file on disk forever otherwise. Anything
# still "uploading" after this long gets reaped on the next upload/start.
...
def _reap_stale_uploads():
    ...
    with JOBS_LOCK:
        for jid, job in JOBS.items():
            if job["status"] == "uploading" and now - job.get("started_at", now) > STALE_UPLOAD_SECONDS:
                stale_ids.append(jid)
        for jid in stale_ids:
            job = JOBS.pop(jid)
            _cleanup(jid, job.get("in_path"))
